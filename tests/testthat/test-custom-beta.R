test_that("test-custom-beta", {
  ...
  # Check lengths
  expect_equal(n, length(dbeta_custom(x, mu = 0.8, phi = 2)))
  expect_equal(n, length(rbeta_custom(n, mu = 0.8, phi = 2)))

  # Compare density and to built-in
  for (mu in mu_list) {
    for (phi in phi_list) {
      expect_eps(
        dbeta_custom(x, mu = mu, phi = phi),
  ...
  expect_error(rbeta_custom(100, 0.8)) # to few arguments
  expect_error(rbeta_custom(10, 2, 3, 4, 5)) # to many arguments
  expect_error(rbeta_custom(-1, mu = 0.8, phi = 2)) # number of drawn samples cannot be smaller 0
  expect_warning(expect_error(rbeta_custom("r", mu = 0.8, phi = 2))) # non-numeric arguments are disallowed
  expect_error(rbeta_custom(100, mu = 1, phi = 2)) # mu must be between 0 and 1
  expect_error(rbeta_custom(100, mu = 0.8, phi = -1)) # phi is not allowed to be negative
})
