...
describe("E2: prediction metrics require a test set", {
  it("pred_rmse_metric returns NA when no test set", {
    ...
    out <- compute_metric(pred_rmse_metric(), fit_result, data_bundle, context, list(task_id = "t"))
  ...
  it("pred_mae_metric computes on the test set when present", {
    ...
    out <- compute_metric(pred_mae_metric(), fit_result, data_bundle, context, list(task_id = "t"))
