...
test_that("custom-softplusnormal", {
  ...
  expect_eps(0.3922206, bayesim::dsoftplusnormal(x = 0.5, mu = 1, sigma = 2), eps = 1e-6)
  ...
  for (outer in 1:n_small) {
    for (inner in 1:n_small) {
      mu <- mus[outer]
      sigma <- sigmas[inner]
      expect_eps(bayesim::dsoftplusnormal(x, mu, sigma), pdf_ref[[outer, inner]], eps, relative = TRUE)
    }
  }
  ...
  test_rng(
    rng_fun = bayesim::rsoftplusnormal, metric_mu = median, n = n_rng, mu_list = mus, aux_par = sigmas,
