...
describe("F1 preflight", {
  it("reports task count, grid shape, and unmet needs", {
    config <- simulation_config(
      data_grid = data.frame(n = c(20, 40)), fit_grid = data.frame(model = "lm"),
      data_generator = .gen, fitter = LinearRegressionFitter(n_draws = 50L),
      metrics = list(rmse_test_metric()), n_replicates = 2L, seed = 1L
    )
  ...
  it("condensed mode runs without error", {
    config <- simulation_config(
      data_grid = data.frame(n = 20), fit_grid = data.frame(model = "lm"),
      data_generator = .gen, fitter = LinearRegressionFitter(n_draws = 50L),
      metrics = list(), n_replicates = 1L, seed = 1L
    )
...
describe("F2 failed_tasks", {
  it("returns an empty tibble when all tasks succeed", {
    config <- simulation_config(
      data_grid = data.frame(n = 20), fit_grid = data.frame(model = "lm"),
      data_generator = .gen, fitter = LinearRegressionFitter(n_draws = 50L),
      metrics = list(), n_replicates = 2L, seed = 1L
    )
...
describe("F3 as_tibble", {
  it("returns the summary tibble", {
    config <- simulation_config(
      data_grid = data.frame(n = 20), fit_grid = data.frame(model = "lm"),
      data_generator = .gen, fitter = LinearRegressionFitter(n_draws = 50L),
      metrics = list(posterior_summary_metric()), n_replicates = 2L, seed = 1L
    )
    res <- run_simulation(config, resume = "never", progress = FALSE)
