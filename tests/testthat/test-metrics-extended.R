...
make_fixture <- function(
  ...
  if (is.null(draws)) {
    draws <- matrix(
      c(rnorm(200, mean = 1), rnorm(200, mean = 2)),
      ncol = 2,
      dimnames = list(NULL, c("b_x", "b_Intercept"))
    )
  }
  if (is.null(true_params)) {
    true_params <- c(b_x = 1, b_Intercept = 2)
  }
  if (is.null(vars_of_interest)) {
    vars_of_interest <- c("b_x", "b_Intercept")
  }
  list(
...
describe("M5 extended metrics", {
  ...
  it("pos_prob_metric computes fraction of positive draws", {
    draws <- matrix(
      c(rep(1, 80), rep(-1, 120)),
      ncol = 1,
      dimnames = list(NULL, "b_x")
    )
    fx <- make_fixture(
      draws = draws,
      true_params = c(b_x = 0),
  ...
  it("posterior_summary_metric returns mean/median/sd/quantiles", {
    ...
    expect_equal(
      out$median[["b_Intercept"]],
      median(fx$fit_result$draws[, "b_Intercept"])
    )
  ...
  it("rank_metric ranks are ~uniform on 0..S for independent draws (chi-square)", {
    ...
    bins <- cut(
      ranks,
      breaks = seq(0, S + 1L, length.out = 11L),
      include.lowest = TRUE
    )
  ...
  it("rmse_loo_metric degrades to NA without PSIS/epred context (F3)", {
    ...
    fx <- make_fixture(
      loo = list(elpd = -6, pointwise = cbind(elpd_loo = c(-1, -2, -3)))
    )
    out <- compute_metric(
      rmse_loo_metric(),
      fx$fit_result,
