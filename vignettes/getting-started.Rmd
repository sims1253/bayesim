...
Create a simulation configuration with your data grid, fit grid, and other parameters. Note that metrics should be passed as a list of Metric objects:
...
config <- simulation_config(
  ...
  metrics = list(RmseMetric(), BiasMetric()),  # Pass Metric objects
  n_replicates = 10L,
  seed = 42L
)
```
...
config <- simulation_config(
  ...
  data_generator = my_data_generator,
  fitter = my_fitter,
  metrics = list(RmseMetric()),  # Pass Metric objects
  n_replicates = 1000L,
  seed = 42L,
...
config <- simulation_config(
  ...
  data_generator = my_data_generator,
  fitter = MockFitter(),
  metrics = list(MyMetric()),  # Use your custom metric
  n_replicates = 5L,
  seed = 42L
)
```
