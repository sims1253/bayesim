...
observations in columns, matching the brms/loo convention.
`validate_fitter(smoke_test = TRUE)` checks these dimensions.
...
This example fits a linear model by OLS and draws coefficients from a normal
approximation using `coef()` and `vcov()`. It holds the residual variance
fixed. Use `LinearRegressionFitter()` for conjugate Bayesian inference.
...
tears them down afterwards. With `workers = 1`, no
