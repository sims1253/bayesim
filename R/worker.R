...
#' The PSIS tail smoothing runs only once:
...
build_loo_context <- function(fitter, fit_result, need_psis = FALSE) {
  ...
  # Reuse the summary's PSIS object when its weights match the draw and
  # observation counts. Older loo versions store log_weights as lw.
  psis_obj <- loo_result[["psis_object"]]
  if (
    !inherits(psis_obj, "psis") ||
      # Short-circuit before [[ when the fitter returns an atomic value.
      !identical(
