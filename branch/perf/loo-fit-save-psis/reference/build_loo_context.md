...
consistent and avoids computing each twice per task (#73). Since \#76
the PSIS tail smoothing itself also runs only once:
[`loo_fit()`](https://sims1253.github.io/bayesim/reference/loo_fit.md)
is called with `save_psis = TRUE` and its returned `psis_object` —
identical to `loo::psis(-ll, r_eff)` on the same matrix — is reused
directly; the direct
[`loo::psis()`](https://mc-stan.org/loo/reference/psis.html) route
remains as a fallback for fitters that return no `psis_object` (e.g.
custom or mock fitters).
